...
check_new_ver(){
    ...
    版本列表请去这里获取：${Green_font_prefix}[ https://github.speed17.workers.dev/depfish/brook/releases ]${Font_color_suffix}"
    read -e -p "直接回车即自动获取:" brook_new_ver
    if [[ -z ${brook_new_ver} ]]; then
        brook_new_ver=$(wget -qO- https://apigithub.speed17.workers.dev/repos/depfish/brook/releases| grep "tag_name"| head -n 1| awk -F ":" '{print $2}'| sed 's/\"//g;s/,//g;s/ //g')
...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "https://github.speed17.workers.dev/depfish/brook/releases/download/${brook_new_ver}/brook_linux_amd64"
        mv brook_linux_amd64 brook
    else
        wget --no-check-certificate -N "https://github.speed17.workers.dev/depfish/brook/releases/download/${brook_new_ver}/brook_linux_386"
