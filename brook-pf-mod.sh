...
#   Description: Brook
#   Version: 1.0.1
#   Author: Toyo, yulewang(DDNS features), depfish(CNAME, iptables)
...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "https://github.speed17.workers.dev/txthinking/brook/releases/download/${brook_new_ver}/brook_linux_amd64"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate https://githubusercontent.speed17.workers.dev/depfish/brook/master/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate https://githubusercontent.speed17.workers.dev/depfish/brook/master/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
